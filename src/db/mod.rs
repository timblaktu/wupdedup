...
impl Bucket {
    ...
    pub fn put_with_indexes(&self, key: &str, value: &[u8], metadata: Option<FileMetadata>) -> Result<()> {
    ...
    pub fn find_duplicates(&self) -> Result<HashMap<String, Vec<String>>> {
        ...
        for item in hash_idx.iter()? {
            ...
            if let Some(colon_pos) = composite.find(':') {
                ...
                hash_files.entry(hash.to_string())
...
mod tests {
    ...
    fn test_indexed_operations() {
        ...
        bucket.put_with_indexes("file1.jpg", b"content1", Some(metadata1)).unwrap();
        ...
        bucket.put_with_indexes("file2.png", b"content2", Some(metadata2)).unwrap();
        ...
        bucket.put_with_indexes("file3.jpg", b"content3", Some(metadata3)).unwrap();
    ...
    fn test_index_performance() {
        ...
        for i in 0..1000 {
            let metadata = FileMetadata {
                ...
                file_type: Some(if i % 2 == 0 { "image/jpeg" } else { "image/png" }.to_string()),
            };
            bucket.put_with_indexes(
                &format!("file_{}.jpg", i),
