...
async fn test_scan_persistence_across_sessions() -> Result<()> {
    ...
    // Reopen database and verify data persisted
    {
        ...
        let key = root_path
            .join("photos/vacation/beach.jpg")
            .to_string_lossy()
            .to_string();
...
async fn test_concurrent_scans_different_buckets() -> Result<()> {
    ...
    let (result1, result2) = tokio::join!(context1.scan_tree(), context2.scan_tree());
...
async fn test_scan_handles_io_errors_gracefully() -> Result<()> {
    ...
    fixture.create_files(&[("normal1.txt", b"content1"), ("normal2.txt", b"content2")])?;
